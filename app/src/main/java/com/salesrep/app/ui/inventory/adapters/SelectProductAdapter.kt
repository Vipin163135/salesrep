...
class SelectProductAdapter (
    val context: Context,
    val onProductClick: (InventoryDataObject?) -> Unit
) :
    RecyclerView.Adapter<SelectProductAdapter.ViewHolder>() {

    private var productList = listOf<InventoryDataObject>()
    ...
    fun notifyData(list: List<InventoryDataObject>?) {
    ...
    class ViewHolder(val binding: ItemAddProductBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(productAssortment: InventoryDataObject) {
