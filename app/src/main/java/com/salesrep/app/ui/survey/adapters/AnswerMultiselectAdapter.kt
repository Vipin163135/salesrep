...
import com.salesrep.app.databinding.ItemAnswerMultiselectBinding
import com.salesrep.app.ui.survey.AnswerClickListener

class AnswerMultiselectAdapter(
    val callBack: AnswerClickListener,
    val question: Question,
    val isClickable: Boolean?= true
) :
    RecyclerView.Adapter<AnswerMultiselectAdapter.ViewHolder>() {

    private var answerList = listOf<Answer>()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding: ItemAnswerMultiselectBinding = DataBindingUtil.inflate(
            LayoutInflater.from(parent.context),
            R.layout.item_answer_multiselect, parent, false
        )
...
//        holder.bind(answerList[position])
   ...
   inner class ViewHolder(val binding: ItemAnswerMultiselectBinding) : RecyclerView.ViewHolder(binding.root) {
